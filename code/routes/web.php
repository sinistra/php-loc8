...
Route::get('/loc8/base_qry/{search_str}/{ret_limit}', function ($search_str, $ret_limit) {
    ...
    $found_array[] = ["nbn_addr" => $base_addr_str];
    foreach ($hits as $hit) {
        $nbn_addr = $hit->_source->official_nbn_address;
        $found_array[] = ["nbn_addr" => get_sub_address($nbn_addr), "nbn_loc" => $hit->_source->nbn_locid, "geo" => $hit->_source->geo_location];
    }
...
{
    $curl_url = "laradock_elasticsearch_1:9200/pfl/_search/";
    //$qry_data = '{ "from" : 0, "size" : ' . $res_limit . ', "query": { "match": { "base_hash": { "query": "' . $search_str . '", "operator": "and" } } } }';
