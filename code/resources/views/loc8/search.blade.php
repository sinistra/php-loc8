...
<html lang="{{ app()->getLocale() }}">
    <head>
		...
		<style>
			...
			#suggest_input { color: black; }
			#suggest_container { color: black; width: 90%; margin-left: 5%; margin-top: 12px; }
			#header_div { width: 100%; height: 50px; border: 0px blue solid; background: rgb(0,0,0); }
			#logo_div { float: left; width: 300px; border: 0px pink solid; }
			#searchbar_div { float: right; width: calc(100% - 300px); height: 100%; border: 0px cyan solid; }
			#main_wrapper { width: 100%; height: calc(100% - 50px); border: 0px red solid; }
			#left_pane_wrapper { letter-spacing: 1px; font-weight: 200; font-size: 10px; color: white; background: #292929; float: left; width: 320px; height: 100%; border: 0px green solid; }
			#welcome_pane { border: 0px white solid; padding: 12px 20px 12px 20px; }
			#results_pane { display: none; border: 0px white solid; padding: 12px 20px 12px 20px; }
			#results_area { margin-left: 10px; }
			#at_address_pane { display: none; border: 0px white solid; border-top: 1px #545454 solid; padding: 10px 20px 12px 20px; }
			#at_geo_pane { display: none;border: 0px white solid; border-top: 1px #545454 solid; padding: 12px 20px 12px 20px; }
			#nearby_pane { display: none; border: 0px white solid; border-top: 1px #545454 solid; padding: 12px 20px 12px 20px;}
			#content_div { float: right; width: calc(100% - 320px); height: 100%; border: 0px orange solid; }
			#map { height: 100%; z-index: 0; }

			.pane_tree { padding: 7px 0px 0px 0px; }
			.results_h1 { margin-top: 5px; font-size: 14px; }
			.results { margin-left: 10px; }
			.modal-content { letter-spacing: 1px; font-weight: 200; font-size: 10px; color: white; background: #292929;}
			.modal-title { letter-spacing: 1px; font-weight: 200; font-size: 15px; font-family: "Helvetica Neue",Helvetica,Arial; }
			.modal-body { height: 60%; overflow-y: auto; }
	    </style>
    ...
    <body>
        	...
        	<div id="searchbar_div">
        		<div id="suggest_container">
		        	...
		        	<div class="input-group input-group-sm mb-3">
					  <input id="suggest_input" type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm" style="font-size: 11px; padding: 4px 12px;">
					    <div class="input-group-append">
						    <div class="input-group-text" style="padding : 0px 5px; height: 26px;">
						  	...
						  	<button id="suggest_button" class="btn btn-outline-secondary" type="button" style="font-size: 13px; padding: 2px 10px; border-radius: 0px 14px 14px 0px;">search</button>
						</div>
					</div>
        ...
        <div id="main_wrapper">
        	<div id="left_pane_wrapper" style="overflow: auto;">
        		<div id="welcome_pane">
        			...
        			<p>LOC-8 (pronounced locate) is here to help you match customer addresses to official (mostly NBN) servicable locations. Simply start typing in the search bar above to see match suggestions.</p>
        			<br><p>For help, questions, or feature suggestions you can contact michael from the locate team at; mhulowatyi@macquarietelecom.com.</p>
        			<br><p>All the best from the LOC-8 team.</p>
        			<br><p><a href="/loc8/visualise">LOC-8</a> responsibly!</p>
        		</div>
        		<div id="results_pane">
		...
		$(document).ready(function() {
			...
			function smoothZoom (map,max,cnt) {
			    if (cnt >= max) {
			        ...
			        setTimeout(function(){map.setZoom(cnt)}, 200); // 80ms sleep between each zoom for smooth overall zoom
			    }
			}
			...
			function updateFoundPin(geoLat,geoLon,searchType) {
				...
				if (searchType == 'zoom') {
					...
					setTimeout(function(){smoothZoom(map, 19, map.getZoom())}, 150);
				}
				else {
					map.panTo(foundLoc);
				}
			...
			function doSelectThings(searchType) {
				...
				var zoom_type = 'zoom';
				if (searchType == 'nearby') { zoom_type = ''; }
		...
		<script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDcE2tHUuIsXqNLwIgtoJ16D-N5b1F7XFM&callback=initMap"></script>
    </body>
</html>